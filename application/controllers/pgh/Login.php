...
class Login extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->masterpage->addContentPage("pgh/login", 'content', $data);
    ...
    public function register() {
        ...
        $this->masterpage->setMasterPage("pgh/MasterPage");
        $this->masterpage->addContentPage("pgh/register", 'content', $data);
        $this->masterpage->show();
    }
        ...
        if (empty($login_ok)) {
            ...
            if ($pw == $login_ok[0]["USER_PW"]) {
				...
				if(!empty($this->login_m->adm_chk($id))){
					//관리자인 경우
					echo "<script>";
					echo "location.href='/pgh/survey/sv_doc_add';";
					echo "</script>";
				} else {
					// 비밀번호가 일치하는 경우
					echo "<script>";
